---
name: build-a-scorer
description: Use when designing or revising a suite of quality checks, evals, or scorers for an LLM app, agent, multi-agent system, RAG app, or GenAI workflow — picking which criteria to measure and deciding rule/code check vs. built-in vs. LLM judge for each. Also use when a judge suite has grown vague, bloated, or expensive, when someone reaches for an LLM judge to check something code could check, or before writing any evaluation criteria. MLflow-native in its API examples; the method is stack-independent and the skill continues from first principles when MLflow is absent.
allowed-tools: Read, Write, Bash, Grep, Glob
---

# build-a-scorer

<!-- Source: https://github.com/mlflow/skills (Apache-2.0, mlflow org, joshuawong-db).
     Installed 2026-08-19. BODY IS VERBATIM — re-pull from upstream rather than editing it.
     ONE LOCAL CHANGE: the frontmatter `description` was rewritten to trigger on eval-suite
     design generally, not on MLflow. Upstream's version gated discovery behind a vendor
     keyword this user doesn't use. Re-apply this edit after any upstream pull.
     Often used here WITHOUT MLflow: Phases 1/3/5 and the Phase 4 routing tree are
     stack-independent. When MLflow is absent, follow the skill's own instruction —
     continue from first principles; treat the built-ins catalog as unavailable, not as
     license to hand-roll a judge without first looking for a cheaper check.
     Companion: llm-judge-alignment validates any judge this produces (its doctrine 8). -->

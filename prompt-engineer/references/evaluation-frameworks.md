...
From base to peak of the evaluation pyramid:

1. **Exact Match / Metrics** (Accuracy, F1, BLEU) — quick sanity checks, baseline comparison
2. **Automated Test Suites** (Regression/Smoke) — fast, repeatable, CI/CD integration
3. **Human Evaluation** (Gold Standard) — expert assessment, ground truth creation
4. **LLM-as-Judge Evaluation** — automated quality scoring, nuanced assessment

...
Baseline Prompt → Measure Results → Diagnose Issues → Change One Thing → (iterate until target met)
...
Prompt underperforms → What's the failure mode?

- **Format issues** → Add output scaffolding, add examples
- **Wrong content** → Improve instructions, add context, use CoT
- **Inconsistent** → Add examples, lower temperature, add constraints

---
name: review-plan
description: Structured adversarial review of a plan or spec across 4 axes (architecture, code quality, tests, performance) before any code is written. Use when a written plan, spec, or implementation doc needs to be pressure-tested before implementation starts — e.g. "review this plan", "poke holes in this plan", "is this plan ready to build". Inspired by Garry Tan's plan-mode workflow.
---

# Review Plan Before Implementation

Review the plan thoroughly before making any code changes. For every issue or recommendation, explain the concrete tradeoffs, give an opinionated recommendation, and ask for user input before assuming a direction.

If the user did not specify which plan to review, ask for the plan file or paste before starting.
...
- Pairs with the superpowers `writing-plans` skill: write the plan, then run this against it before `executing-plans`

## Sources

- Inspired by [Garry Tan's Plan Mode prompt](https://garrytan.com/) (Feb 2026)
- Adapted from [FlorianBruniaux/claude-code-ultimate-guide](https://github.com/FlorianBruniaux/claude-code-ultimate-guide/blob/main/examples/skills/review-plan/SKILL.md)

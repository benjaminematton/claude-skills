...
**Skills that fire from context (and can be chained):** `brainstorming`, `grilling`, `domain-modeling`, `codebase-design`, `prototype`, `writing-plans`, `subagent-driven-development`, `executing-plans`, `test-driven-development`, `systematic-debugging`, `code-review`, `review-plan`, `verification-before-completion`, `finishing-a-development-branch`, `using-git-worktrees`, `wizard`, `resolving-merge-conflicts`, `prompt-engineer`, `become-expert`, `frontend-design`, `web-design-guidelines`, `receiving-code-review`, `linear-delegation`, `simplify`, `handoff`.
